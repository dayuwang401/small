...
from agents.reasoning_agent_v2 import ReasoningAgent_v2
from agents.code_agent_v2 import CodeAgent_v2
...
class GRPOTrainer:
    def __init__(self, reasoning_model_path, code_model_path):
        ...
        try:
            # 推理模型及其副本
            self.base_reasoning_model = ReasoningAgent_v2(reasoning_model_path, device=self.reasoning_device, dtype=torch.float32).model
            self.reasoning_agent = ReasoningAgent_v2(reasoning_model_path, device=self.reasoning_device, dtype=torch.float32)
